...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET || 'seu_secret_temporario', (err, user) => {
...
app.get('/api/cart/:sessionId', async (req, res) => {
  try {
    ...
    console.log('Buscando carrinho para sessionId:', sessionId);
    ...
    const validItems = items.filter(item => {
      if (!item.productId) {
        console.warn('Item sem produto:', item._id);
        return false;
      }
      return true;
    });
